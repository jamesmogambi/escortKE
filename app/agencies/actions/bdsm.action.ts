...
export interface FetchBDSMEscortsParams {
  county?: string;
  region?: string;
  practice?: string; // Specific BDSM practice (e.g., "domination", "submission", "bondage")
  page?: number;
  limit?: number;
...
export interface FetchBDSMEscortsResponse {
  ...
  filterOptions: {
    practices: string[]; // All available BDSM practices
    counties: Array<{ name: string; count: number }>;
    priceRange: { min: number; max: number };
  };
  popularPractices: Array<{
    name: string;
    count: number;
    avgRating: number;
  }>;
...
export async function getBDSMEscorts(
  params: FetchBDSMEscortsParams = {},
): Promise<FetchBDSMEscortsResponse> {
  try {
    await connectToDB();
    ...
    // Build the base query - must have BDSM services
    const query: any = {
      gender,
      isActive: true,
      role: "escort",
      // Must have at least one BDSM practice
      bdsm: { $exists: true, $ne: [] },
    };

    // Filter by specific BDSM practice (case-insensitive)
    if (practice) {
      query.bdsm = {
        $in: [new RegExp(practice, "i")],
      };
    }
    ...
    const [escorts, total] = await Promise.all([
      Escort.find(query)
        ...
        .select({
          ...
          // Services - Include BDSM practices
          bdsm: 1,
          practices: 1,
          categories: 1,
          extraServices: 1,
    ...
    // Format escorts for client
    const formattedEscorts = escorts.map((escort) =>
      formatEscortToCardData(escort),
    );

    // Get filter options for the sidebar/filters
    const filterOptions = await getBDSMFilterOptions({
      county,
      region,
      gender,
    });

    // Get popular BDSM practices
    const popularPractices = await getPopularBDSMPractices(query);
    ...
    console.error("Error fetching BDSM escorts:", error);
    throw new Error("Failed to fetch BDSM escorts");
  }
}
...
async function getBDSMFilterOptions(filters: {
  county?: string;
  region?: string;
  gender: string;
}) {
  try {
    ...
    // Get all distinct BDSM practices
    const bdsmPractices = await Escort.distinct("bdsm", baseMatch);

    // Filter out empty values and sort
    const filteredPractices = bdsmPractices
      .filter((practice: string) => practice && practice.trim() !== "")
      .sort();
    ...
    console.error("Error getting BDSM filter options:", error);
    return {
      practices: [],
      counties: [],
...
async function getPopularBDSMPractices(baseQuery: any): Promise<
  Array<{
    name: string;
    count: number;
    avgRating: number;
...
> {
  try {
    // Remove practice filter from base query for popular practices
    const popularQuery = { ...baseQuery };
    delete popularQuery.bdsm;
    ...
    console.error("Error getting popular BDSM practices:", error);
    return [];
  }
}

// ============ GET SINGLE BDSM ESCORT BY ID ============

export async function getBDSMEscortById(
  id: string,
  ...
  try {
    ...
    await Escort.findByIdAndUpdate(id, { $inc: { totalViews: 1 } });

    return formatEscortToCardData(escort);
  } catch (error) {
    console.error("Error getting BDSM escort by ID:", error);
    return null;
  }
}

// ============ GET SIMILAR BDSM ESCORTS ============

export async function getSimilarBDSMEscorts(
  escortId: string,
  limit: number = 4,
  ...
  try {
    ...
    const escort = await Escort.findById(escortId).select("bdsm gender county");
    ...
    const query: any = {
      ...
      isActive: true,
      role: "escort",
      bdsm: { $in: escort.bdsm?.slice(0, 3) || [] },
    };

    if (escort.county) {
      query.county = escort.county;
    }

    const similar = await Escort.find(query)
      .populate("countyDetails", "name")
    ...
    return similar.map((s) => formatEscortToCardData(s));
  } catch (error) {
    console.error("Error fetching similar BDSM escorts:", error);
    return [];
  }
}

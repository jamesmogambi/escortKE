...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Store region in database using the consistent RegionService
    if (storeRegion) {
      try {
        regionDoc = await RegionService.getOrCreateRegion(region, county, {
          notes: `Auto-created from RahaEscorts scraping`,
        });
        console.log(`📌 Region: ${regionDoc.name} (ID: ${regionDoc.id})`);
      } catch (regionError) {
      ...
      const batchPromises = batch.map(async (escort) => {
        ...
        try {
          ...
          if (result.success) {
            ...
            console.log(
              `  ✅ Saved: ${fullDetails.name || escort.name} (Age: ${fullDetails.age || "N/A"}, Gender: ${fullDetails.gender}, Images: ${fullDetails.images?.length || 0}, Region: ${regionDoc?.name || region})`,
            );
          } else {
            totalFailed++;
    ...
    - Region: ${regionDoc?.name || region} (ID: ${regionDoc?.id || "N/A"})
    `);

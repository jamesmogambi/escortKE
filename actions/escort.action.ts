...
export async function fetchGirlEscorts(
  ...
  try {
    ...
    // Execute main query with populate
    const [escorts, total] = await Promise.all([
      Escort.find(query)
        ...
        .select({
          ...
          videos: 1,
          // FIXED: Use $substrCP instead of $substr to handle UTF-8 characters
          about: { $substrCP: ["$about", 0, 200] },
          ethnicity: 1,
          nationality: 1,
          bustSize: 1,
          weight: 1,
...
function formatEscortForClient(escort: any): EscortCardData {
  ...
  // Safely truncate about text in JavaScript as backup
  const aboutText = escort.about || null;
  const aboutExcerpt = aboutText
    ? aboutText.length > 150
      ? Array.from(aboutText).slice(0, 150).join("") + "..."
      : aboutText
    : null;
...
function getSortOptions(sortBy: string): any {
  switch (sortBy) {
    ...
    case "price_low":
      // This requires a more complex aggregation, but for now use a simple sort
